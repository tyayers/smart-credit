...
function setCreditLength(length) {
    creditLength = length;
    calculate();
}
...
function calculate() {
    ...
    if (amount) {
        var monthlyAmount = parseFloat(amount) / creditLength;
            
        if ($("#homeMaintenanceCheck").is(":checked")) {
            monthlyAmount = monthlyAmount - 4.17;
        }
...
function homeMaintenenaceChange(e) {
    calculate();
}
